        ...
        {blogs.length > 0 ? (
          <div className="space-y-4">
            <div className="space-y-8">
              {
                blogs.map((blog) => (
                  <motion.div onClick={() => { setSelectedBlog(blog); router.push(`/story/${blog._id}`) }}
                    key={blog._id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                    transition={{ duration: 0.3 }}
                    className="rounded-2xl hover:scale-101 cursor-pointer h-fit p-2 shadow-md hover:shadow-lg transition flex items-center gap-4"
                  >
                    <div className="flex-1">
                      <h2 className="text-lg font-semibold text-gray-900 mb-1">
                        {blog.title || "Blog Title Goes Here"}
                      </h2>
                      <p className="text-gray-600 text-sm line-clamp-3 mb-3" dangerouslySetInnerHTML={{ __html: blog.description }} />
                    ...
                    <img
                      src={blog.image || "https://tse2.mm.bing.net/th/id/OIP.90sDWdblfZFiciIEpsGFwwHaEY?rs=1&pid=ImgDetMain&o=7&rm=3"}
                      alt={blog.title || "Blog cover"}
                      className="w-40 h-32 scale-95 object-cover rounded-xl"
                    />
                  </motion.div>
                ))
              }
